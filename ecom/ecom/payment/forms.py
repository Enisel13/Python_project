...
#forms.ModelForm- we are taking the form and accessing the model the database
#Here we use forms.Form instead because we are going to collect credit card information and we really don`t want to store 
#this credit card information on our server because we are going to connect to stripe in order to process the order
#and so we want to send that information to stripe we don`t want to save it bc there are all kinds of laws involved with saving 
#credit card numbers on your web server in your database 
class PaymentForm(forms.Form):
    ...
    #We don`t need a class Meta because we are not picking a model
